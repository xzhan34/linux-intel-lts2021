...
static void test_produce_mappable(struct kunit *test)
{
	...
	KUNIT_ASSERT_LT(test, data_chunk, data_len);
	...
	KUNIT_EXPECT_EQ(test,
			memcmp(buf + sizeof(data->hdr), priv->resource.vaddr, priv->resource.size),
			0);
}
